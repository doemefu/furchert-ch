...
export const authConfig = {
  ...
  session: {
    strategy: 'jwt',
    // 7 days, matching auth-service's `app.jwt.refresh-token-expiry` — the
    // window after which `TokenCleanupScheduler` purges the `oauth2_authorization`
    // row an `id_token_hint` needs to resolve at RP-initiated logout (see
    // ../auth-service/INTERFACES.md §1 "RP-Initiated Logout" and "Important
    // Notes" #2). Previously unset (Auth.js default: 30 days), which let this
    // session outlive the IdP-side authorization by 23 days (#30). `updateAge`
    // is left at the Auth.js default: a continuously-active session still
    // slides its `exp` forward indefinitely — a documented limitation, not
    // fixed here (see federated-logout/route.ts for how a stale-but-present
    // `id_token` is handled at logout time).
    maxAge: 7 * 24 * 60 * 60,
  },
  providers: [
    {
